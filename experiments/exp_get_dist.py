...
class dist_Experiment(BaseExperiment):
    def __init__(self, config, core, exp):
        ...
        self.path_planner = PathPlanning(self.env_map)
    ...
    def apply_actions(self, actions, core):
        ...
        while True:
            ...
            for i in range(1):
                ...
                new_pos = self.env_map.get_cartesian_node_position(5)
                new_pos[2] = 10
                self.formation.execute(self.actor_groups[i], new_pos, current_pos, 'solid')
                self.calc_dist[f'actor_{i}'] += self.formation.speed[0]
                
                if np.round(current_pos[0], 1) == np.round(new_pos[0], 1) and np.round(current_pos[1], 1) == np.round(new_pos[1], 1):
